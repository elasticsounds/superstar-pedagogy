...
function attachWordHighlighter(audio) {
  ...
  if (!timecodeData || !timecodeData[dataId]) return;
  const entry = timecodeData[dataId];
  ...
  // Get the translated text directly from translations or fall back to element content
  const translatedText = element.getAttribute('data-id') && 
                         state.translations && 
                         state.translations[element.getAttribute('data-id')] 
                         ? state.translations[element.getAttribute('data-id')] 
                         : element.textContent;

  console.log(`Audio highlighter using text: "${translatedText}" for element with data-id: ${dataId}`);
